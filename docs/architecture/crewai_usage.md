...
**Single source of truth** - defines all agents, tasks, and MCP tool mappings using CrewAI's native DSL.

Features:
- Declarative agent definitions with role-based tool filtering
- Native MCP integration (`mcp://server/tool` syntax)
- Hierarchical process with Technical Director as manager
- No custom Python adapters needed

### .mcp.json (root)
Generated by `ruler apply --nested` from `.ruler/ruler.toml`. Shared by all agents (IDE + CrewAI).
