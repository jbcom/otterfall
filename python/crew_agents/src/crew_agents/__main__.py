...
print("🚀 Initializing Rivermarsh Game Development Crew (MCP Native)")
print("=" * 70)
...
def setup_mcp_servers():
    ...
    No custom adapters needed - CrewAI auto-converts MCP tools.
    """
    mcp_config_path = Path("crew_config/mcp_servers.json")
    ...
    print("✓ Loaded MCP servers from crew_config/mcp_servers.json")
...
def create_agents(agents_config: dict, llm, all_tools: list) -> dict:
    ...
    for agent_name, config in agents_config.items():
        ...
        print(f"✓ Created agent: {agent_name} ({len(agent_tools)} tools)")
...
def create_tasks(tasks_config: dict, agents: dict) -> list:
    ...
    for task_name, config in tasks_config.items():
        ...
        if not agent_name or agent_name not in agents:
            print(f"⚠️  Skipping task {task_name}: agent '{agent_name}' not found")
            continue
...
def main():
    ...
    print("\n" + "=" * 70)
    print("🎮 Starting Rivermarsh Development Crew")
    print("=" * 70)
